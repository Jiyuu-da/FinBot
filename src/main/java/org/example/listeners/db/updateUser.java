...
public class updateUser extends ListenerAdapter {
    @Override
    public void onSlashCommandInteraction(SlashCommandInteractionEvent event) {
        ...
        if((dev_kira_ID.equalsIgnoreCase("576834455306633216" ))|| (dev_kira_ID.equalsIgnoreCase("677251211187060751"))) {
            if(command.equalsIgnoreCase("updateyt")) {
                ...
                try {
                    LocalDate dueDate = DBSetup.getDueDate(name);
                    DBSetup.updateUserAmount(name, amount, lastPayment, dueDate, advance_months);
